...
The 75 detectors fall into six audit families:
...
The suite's evaluation evidence and its current size are **two separate facts** — they are reported at different versions, and should not be collapsed into a single "75 detectors, validated by E1/E7" claim.

- **Current detector catalog: 75** (the enumerated list in `metadata/detectors_catalog.json`).
