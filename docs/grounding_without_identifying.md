...
**The judgement** — `scripts/llm_provenance_review.sh`, and `scripts/install_git_hooks.sh` to make
it run on every push rather than when someone remembers. At pre-push it reads the **added lines**,
not only paragraphs: a table row and a bullet list are neither, and two of the six findings were
exactly those shapes. Fails open when the CLI is absent — most contributors do not have it, and a
checklist typo must never be blocked by a tool nobody has.
