...
a model, which answers the one question a pattern cannot. A router,
...
# because that distinction is semantic. Those route to a reader.
#
...
def main() -> int:
    ...
    ap.add_argument("--mode", choices=("block", "route"), default="route",
                    help="'block': only blocks carrying a BLOCKING signal — what CI refuses. "
                         "'route': every candidate, for a reader.")
    ...
    if not args.quiet:
        if not findings:
            ...
            print("identifying half. A pattern cannot answer that question; a reader can.")
