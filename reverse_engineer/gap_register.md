...
| G44 | (cross-sectional-cohort lane) | paper-driven discovery: acquire strong CC-BY **cross-sectional / prevalence / health-screening** cohort papers from good journals and surface what the suite does not yet cover or check beyond O1–O9 / the cohort gates. Lane seed — specific gaps added as papers are read | 4 | 4 | 3 | 48 | shipped (this PR — surfaced G45/G46) |
| G45 | peer-review + self-review | `observational_confounding.md` **O11 — complex-survey design & weighting** (NHANES/KNHANES/CHNS): design-based estimation (right weight + strata + PSU/Taylor or replicate weights), correct/scaled weight (MEC vs interview; multi-cycle division), subpopulation-domain-not-row-deletion, weighted≠sample-n + design-effect. Found reverse-engineering CC-BY NHANES cross-sectional papers | 5 | 4 | 5 | 100 | shipped (this PR) |
| G46 | peer-review + self-review | `observational_confounding.md` **O12 — data-driven threshold / non-linearity mining**: RCS 'non-linear/saturation' + recursive-search 'inflection point'/'threshold effect' breakpoint reported as a point with no breakpoint CI, no pre-specification, no multiplicity acknowledgement → data-mined cutoff. Needs pre-specified non-linearity test + breakpoint CI + stability. Found in the same NHANES corpus | 4 | 4 | 4 | 64 | shipped (this PR) |
| G47 | peer-review + self-review | `observational_confounding.md` **O13 — cross-sectional mediation** (temporal order & sequential ignorability): a Baron–Kenny / Sobel / PROCESS / bootstrapped indirect-effect chain from single-timepoint data cannot establish X→M→Y; the bootstrap CI addresses sampling variability, not identification; needs an unmeasured-M–Y-confounding sensitivity (E-value for the indirect effect) + temporal-order caveat; proportion-mediated unstable at small total effect. + `exemplar_findings/cross_sectional_mediation.md`. Found reverse-engineering 5 CC-BY cross-sectional mediation papers | 5 | 4 | 5 | 100 | shipped (this PR) |
| G48 | analyze-stats | mediation-analysis guide (`analysis_guides/` or `table-types/`) — the skill has **no** mediation content despite cross-sectional mediation being one of the most-published genres: bootstrapped a×b indirect effect, proportion-mediated with uncertainty, AGReMA reporting, and the design caveat that identification (not the bootstrap) is the issue. Surfaced alongside O13 | 4 | 3 | 5 | 60 | open |
